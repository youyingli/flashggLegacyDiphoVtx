...
VertexProbTrainingTreeMaker::analyze( const edm::Event &iEvent, const edm::EventSetup &iSetup )
{
    ...
    for( size_t idipho = 0; idipho < diphotonPointers.size(); idipho++ ) {
        ...
        diphoInfo.dipho_vtx_index = diphoPtr->vertexIndex();
        ...
        //cout << pvPointers[0]->position().z() << "  " << diphoPtr->vtx()->position().z() << endl;

        if (diphoInfo.nvertex > 1) diphoInfo.dz01 = pvPointers[0]->position().z() - pvPointers[1]->position().z();
        if (diphoInfo.nvertex > 2) diphoInfo.dz02 = pvPointers[0]->position().z() - pvPointers[2]->position().z();
...
VertexProbTrainingTreeMaker::beginJob()
{
    ...
    DiphoTree->Branch( "dipho_vtx_index"      , &diphoInfo.dipho_vtx_index    , "dipho_vtx_index/I"    );
    ...
    DiphoTree->Branch( "dz01"                 , &diphoInfo.dz01               , "dz01/F"               );
    DiphoTree->Branch( "dz02"                 , &diphoInfo.dz02               , "dz02/F"               );
}
